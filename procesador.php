...
if($error==""){
    ...
    if ($success)
    {
        ...
        //GUARDAR A BD YA QUE YA SE ENVIO UN CORREO
        require_once('conexion.php');
        $sqlBuscar = 'select * from registro_usuario where email=:email';
        $preparado2= $conexion->prepare($sqlBuscar);
        $preparado2->execute(array(':email'=>$email));
        $busqueda = $preparado2->fetchAll();
        if ($busqueda==false){
            //SI NO EXISTE REGISTRO DE ESE EMAIL LO GUARDO
            $sql = 'insert into registro_usuario (nombre, email) values (:nombre, :email)';
